...
class QuestionClassifier:
    def __init__(self):
        ...
        #　特征词路径
        self.disease_path = os.path.join(cur_dir, 'dict/disease.txt')       #疾病名称
        self.department_path = os.path.join(cur_dir, 'dict/department.txt') #诊室名称
        self.check_path = os.path.join(cur_dir, 'dict/check.txt')           #检查项目名称
        self.drug_path = os.path.join(cur_dir, 'dict/drug.txt')             #药品名称
        self.food_path = os.path.join(cur_dir, 'dict/food.txt')             #食物名称
        self.producer_path = os.path.join(cur_dir, 'dict/producer.txt')     #药品生产厂家
        ...
        self.prevent_qwds = ['预防', '防范', '抵制', '抵御', '防止', '躲避', '逃避', '避开', '免得', '逃开', '避开', '避掉', '躲开', '躲掉', '绕开','怎样才能不', '怎么才能不', '咋样才能不', '咋才能不', '如何才能不','怎样才不', '怎么才不', '咋样才不', '咋才不', '如何才不','怎样才可以不', '怎么才可以不', '咋样才可以不', '咋才可以不', '如何可以不','怎样才可不', '怎么才可不', '咋样才可不', '咋才可不', '如何可不', '日常', '护理']
        self.lasttime_qwds = ['周期', '多久', '多长时间', '多少时间', '几天', '几年', '多少天', '多少小时', '几个小时', '多少年']
        self.cureway_qwds = ['怎么治疗', '如何医治', '怎么医治', '怎么治', '怎么医', '如何治', '医治方式', '疗法', '咋治', '怎么办', '咋办', '咋治']
        self.cureprob_qwds = ['多大概率能治好', '多大几率能治好', '治好希望大么', '几率','概率' '几成', '比例', '可能性', '能治', '可治', '可以治', '可以医']
        self.getway_qwds = ['传染']
        self.easyget_qwds = ['易感人群', '容易感染', '易发人群', '什么人', '哪些人', '感染', '染上', '得上']
        self.check_qwds = ['检查', '检查项目', '查出', '检查', '测出', '试出']
        self.belong_qwds = ['属于什么科', '属于', '什么科', '科室']
        self.cure_qwds = ['治疗什么', '治啥', '治疗啥', '医治啥', '治愈啥', '主治啥', '主治什么', '有什么用', '有何用', '用处', '用途','有什么好处', '有什么益处', '有何益处', '用来', '用来做啥', '用来作甚', '需要', '要', '可以治']
    ...
    def classify(self, question):
        data = {}
        medical_dict = self.check_medical(question) #获取问句中包含的领域词及其所在领域
        if not medical_dict:
